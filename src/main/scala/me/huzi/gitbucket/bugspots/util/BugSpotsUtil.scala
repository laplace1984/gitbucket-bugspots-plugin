...
object BugSpotUtil {
  ...
  private def getDiffs(git: Git, from: String, to: String): List[DiffInfo] = {
    ...
    git.diff.setNewTree(newTreeIter).setOldTree(oldTreeIter).call.asScala.map { diff =>
      ...
      DiffInfo(diff.getChangeType, diff.getOldPath, diff.getNewPath, None, None, oldIsImage, newIsImage, Option(diff.getOldId).map(_.name), Option(diff.getNewId).map(_.name), diff.getOldMode.toString, diff.getNewMode.toString, false)
    }.toList
  }
